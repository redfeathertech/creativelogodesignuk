import { reviews } from "@/content/landing/lp";
import { Section } from "@/components/ui/Section";
import { StarIcon } from "@/components/ui/icons";

/**
 * The three named reviews.
 *
 * Rendered as a plain grid, not a carousel: the live page shows two at a time
 * and cycles on a 2.5-second timer, so one is always absent from the document.
 *
 * The live card puts the review heading in an `<h4>` **below** the reviewer's
 * `<h5>` name — a level-5 heading followed by a level-4 one inside the same
 * card. The order and the levels are corrected; the text is not.
 *
        ...
        <Section tone="dark">
            <div className="container-site">
                <div className="reveal mx-auto max-w-[70ch] text-center">
                    <h2 className="text-h2">{reviews.title}</h2>
