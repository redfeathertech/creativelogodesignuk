...
/**
 * Tool grid (v2). Four across from `lg:`, three from `md:`, two on phones.
 * Sixteen tools divide evenly by four, so the widest layout has no orphan on
 * the last row.
 *
 ...
 * variant here lost the cascade to `md:grid-cols-3` and pinned the grid at
 * three on desktop. (Do not write the class form of such a variant in a
 * comment — the scanner reads comments too and will emit the rule.)
 *
 * Tool names are <p>-level spans, NOT headings. The live site marks all 16 as
 * <h4>, which is heading spam — "Figma", "CSS" and "Vue" are labels, not
 * section titles. docs/CONTENT-PARITY.md records this as a deliberate fix;
 * reintroducing headings here would quietly undo it.
 */
                ...
                <ul className="grid grid-cols-2 gap-4 md:grid-cols-3 lg:grid-cols-4">
                    {toolbox.tools.map((tool) => (
                        <li
                            ...
                            className="reveal flex flex-col items-start gap-2 rounded-md border border-ink-900/[0.08] bg-white p-5 transition-[translate,box-shadow,border-color] duration-300 ease-out hover:-translate-y-1 hover:border-magenta-500/30 hover:shadow-lg min-[27rem]:flex-row min-[27rem]:items-center min-[27rem]:gap-4"
                        >
                            <Image
                                src={tool.icon}
                                alt=""
                            ...
                            {/* min-w-0 so this can shrink as a flex item, and
                                break-words as the backstop — without both, a name
                                longer than the text column does not wrap, it
                                spills straight out through the card border. */}
                            <span className="min-w-0">
                                <span className="block font-display leading-[1.2] font-bold break-words text-onlight">
                                    {tool.name}
                                </span>
                                <span className="mt-0.5 block text-xs text-onlight-muted">
                                    {tool.kind}
                                </span>
                            </span>
                        </li>
